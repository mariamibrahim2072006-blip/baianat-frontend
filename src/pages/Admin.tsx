...
// 🛑 إيميلك الشخصي المسموح له فقط بفتح هذه اللوحة
const OWNER_EMAIL = 'mariamibrahim2072006@gmail.com';
...
export default function Admin() {
    ...
    // 🔒 حماية صارمة: التحقق من أن المستخدم هو المالك فقط
    useEffect(() => {
        if (!loading) {
            if (!user) {
                navigate('/login');
    ...
    const handleAddProduct = async (e: FormEvent) => {
        ...
        const stock = newStock.trim() ? Number(newStock) : 10;
    ...
    if (loading || !user || user.email !== OWNER_EMAIL) {
        return <div style={{ minHeight: '70vh', display: 'flex', justifyContent: 'center', alignItems: 'center', fontFamily: 'Inter, sans-serif' }}>Loading...</div>;
    }
        ...
        <div style={{ minHeight: '80vh', background: bg, color: text, padding: '40px 0 100px', fontFamily: 'Inter, system-ui, sans-serif' }}>
            <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 20px' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '20px', flexWrap: 'wrap', marginBottom: '35px' }}>
                    <div>
                        <p style={{ color: '#DB4444', fontWeight: '700', fontSize: '13px', margin: '0 0 6px', textTransform: 'uppercase', letterSpacing: '1.2px' }}>ADMIN CONTROL PANEL</p>
                        <h1 style={{ margin: 0, fontSize: '32px', fontWeight: '700', letterSpacing: '-0.5px' }}>Dashboard Overview</h1>
                        <p style={{ color: secondary, margin: '6px 0 0', fontSize: '15px' }}>Welcome back, <strong>{user.username}</strong></p>
                    </div>
                    <button type="button" onClick={() => navigate('/')} style={{ display: 'flex', alignItems: 'center', gap: '8px', border: `1.5px solid ${border}`, background: cardBg, color: text, padding: '12px 20px', borderRadius: '8px', cursor: 'pointer', fontWeight: '600', fontSize: '14px', transition: 'all 0.2s' }}>
                        <ArrowLeft size={16} />
                        Back to Store
                    </button>
                </div>
                ...
                {activeTab === 'dashboard' && (
                    <>
                        ...
                        <div style={{ background: cardBg, border: `1.5px solid ${border}`, borderRadius: '12px', padding: '30px', marginBottom: '30px', boxShadow: '0 4px 20px rgba(0,0,0,0.03)' }}>
                            <h3 style={{ margin: '0 0 20px', fontSize: '20px', fontWeight: '700' }}>Sales Analytics Overview</h3>
                            {orders.length === 0 ? (
                                <p style={{ color: secondary }}>No sales data available yet to display chart.</p>
                            ) : (
                                <div style={{ width: '100%', height: '300px' }}>
                                    <ResponsiveContainer width="100%" height="100%">
                                        <AreaChart data={orders.map((order, index) => ({ name: `Order #${index + 1}`, total: Number(order.total || 0) }))} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
                                            <defs>
                                                <linearGradient id="colorSales" x1="0" y1="0" x2="0" y2="1">
                                                    <stop offset="5%" stopColor="#DB4444" stopOpacity={0.8} />
                                                    <stop offset="95%" stopColor="#DB4444" stopOpacity={0} />
                                                </linearGradient>
                                            </defs>
                                            <CartesianGrid strokeDasharray="3 3" stroke={darkMode ? '#333' : '#e2e8f0'} />
                                            <XAxis dataKey="name" stroke={secondary} />
                                            <YAxis stroke={secondary} />
                                            <Tooltip contentStyle={{ backgroundColor: cardBg, borderColor: border, color: text, borderRadius: '8px' }} />
                                            <Area type="monotone" dataKey="total" stroke="#DB4444" fillOpacity={1} fill="url(#colorSales)" />
                                        </AreaChart>
                                    </ResponsiveContainer>
                                </div>
                            )}
                    ...
                    <>
                        <div style={{ background: cardBg, border: `1px solid ${border}`, borderRadius: '12px', padding: '35px', marginBottom: '30px', boxShadow: '0 4px 20px rgba(0,0,0,0.03)' }}>
                            ...
                            <form onSubmit={handleAddProduct} style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '20px' }}>
                                ...
                                <AdminInput placeholder="Stock Quantity (e.g. 15)" type="number" value={newStock} onChange={setNewStock} background={inputBg} color={text} border={border} />
                                    ...
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '18px' }}>
                                        ...
                                        <div>
                                            ...
                                            <div style={{ display: 'flex', gap: '15px', alignItems: 'center' }}>
                                                <span style={{ color: '#DB4444', fontWeight: '700', fontSize: '15px' }}>${product.price}</span>
                                                <span style={{ fontSize: '13px', fontWeight: '600', padding: '2px 8px', borderRadius: '6px', backgroundColor: (product.stock ?? 0) <= 3 ? '#fee2e2' : '#e9f8ee', color: (product.stock ?? 0) <= 3 ? '#dc2626' : '#10b981' }}>
                    ...
                    <div style={{ backgroundColor: cardBg, color: text, padding: '35px', borderRadius: '12px', width: '100%', maxWidth: '500px', boxShadow: '0 10px 30px rgba(0,0,0,0.2)', border: `1px solid ${border}` }}>
                        ...
                        <form onSubmit={handleUpdateProduct} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                            ...
                            <div>
                                ...
                                <input type="number" value={editStock} onChange={(e) => setEditStock(e.target.value)} required style={{ width: '100%', padding: '12px', backgroundColor: inputBg, color: text, border: `1.5px solid ${border}`, borderRadius: '6px', outline: 'none' }} />
                            </div>
                            <div>
